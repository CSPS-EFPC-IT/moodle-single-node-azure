#!/bin/bash
#
# Implements a package of commonly used functions, namely:
# - add_hosts_file_entry
# - echo_action
# - echo_error
# - echo_info
# - echo_title
...
#######################################
# Permently mount a data disk identified by its size to a mount point.
...
# to use this function to mount a data disk that has the same size as another
# device attached to the server.
# Format the data disk if no file system exists.
# Globals:
#   [List of global variables used and modified, if any.]
# Arguments:
...
#   Writes error messages to STDERR.
#######################################
function mount_data_disk_by_size() {
...
#######################################
# Stop script loudly on any error.
# Globals:
#   $BASH_COMMAND
#   last_command
#   current_command
...
#   Writes error messages to STDERR.
#######################################
function update_apache2_config_file() {
  local parameter="$1"
  local value="$2"
...
#   Writes error messages to STDERR.
#######################################
function update_php_config_file() {
  local parameter="$1"
  local value="$2"
  local config_file_path="$3"
